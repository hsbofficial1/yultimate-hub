...
import { AlertTriangle, CheckCircle2, X, Clock, User } from 'lucide-react';
          ...
          {unacknowledgedAlerts.map((alert) => (
            <Alert key={alert.id} variant="destructive">
              ...
              <AlertTitle className="flex items-center justify-between">
                <span>
                  {alert.children.name} - {alert.consecutive_absences} Consecutive
                  Absences
                </span>
                ...
                {alert.sessions && (
                  <div className="flex items-center gap-4 text-sm text-muted-foreground">
                    <span>
                      ...
                      {format(new Date(alert.sessions.date), 'MMM d, yyyy')}
                    </span>
                    <span>{alert.sessions.location}</span>
                  </div>
                )}
                <div className="mt-2 flex items-center gap-4 text-sm">
                  <span>
                    <User className="h-3 w-3 inline mr-1" />
                    Parent: {alert.children.parent_name}
                  </span>
                  <a
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="font-medium">
                        {alert.children.name} - {alert.consecutive_absences}{' '}
      ...
      {alerts.length === 0 && (
        <Card>
          <CardContent className="py-8 text-center">
            ...
            <p className="text-muted-foreground">No absence alerts</p>
          </CardContent>
        </Card>
      )}
    </div>
